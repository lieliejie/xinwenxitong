<template>
  <div class="pindao-page">
    ...
    <div class="page-header">
      <div class="header-inner">
        ...
        <div class="header-stats">
          <div class="stat-box">
            <span class="stat-val">{{ redian.length }}</span>
            <span class="stat-label">篇精选</span>
          </div>
          <div class="stat-box">
            <span class="stat-val">{{ reyidu()  }}</span>
    ...
    <div class="grid-container">
      <div 
        class="grid-card" 
        v-for="item in redian" 
        :key="item.id"
        @click="yueduliang(item)"
      >
        <div class="grid-img">
          <img :src="item.tuPian" alt="">
        </div>
        <div class="grid-body">
          <h4 class="grid-title">{{ item.biaoTi }}</h4>
          <p class="grid-desc">{{ item.jianJie }}</p>
          <div class="grid-meta">
            <span class="grid-source">{{ item.fenLei }}</span>
            <span class="grid-time">{{ item.riQi }}</span>
...
let store = useStore()
let { yueduliang } = store  


const pinDaodataList = [
  {
    id: 1,
    biaoTi: '重磅政策密集落地，多行业迎全新发展格局',
    jianJie: '今年以来，多项促进实体经济发展的政策措施陆续出台，涵盖制造业、消费、科技等多个领域。',
    fenLei: '热点',
    riQi: '2026-06-08',
    yueduLiang: 12800,
    tuPian: 'https://picsum.photos/seed/hot1/800/400'
  }
]



const redian = computed(() => {
...
let reyidu = ()=>{
  let zonghe = 0
  redian.value.map(item=>{
   zonghe += item.yueduLiang || 0
  })
  return zonghe
}
